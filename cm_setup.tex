...
A Bayesian network classifier~\cite{Pearl1988} is learned to differentiate genuine data from spoofed data in the case of the far-field channel detection countermeasure. {\bfseries This classifier uses a network of probabilistic dependencies between random variables.} In contrast, the AdaBoost M1 meta classifier~\cite{Freund1999} is used for the LBP countermeasure. {\bfseries  This classifier is an algorithm that iteratively uses other 'weak' classifiers, in our case decision trees, and iteratively boosts their performance by exposing them to previously misclassified items.} These two classifiers returned the best results in each case for an area-under-the-ROC metric. {\bfseries Both classifiers were implemented using the Weka toolkit~\cite{Weka}.} %Having been trained, both classifiers were applied to detect replay attacks in both spoofing accesses and licit client trials.
