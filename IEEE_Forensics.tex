...
\title{This Will We the Title of the Paper for the IEEE Transactions on Information Forensics and Security }
%
%
...
\thanks{N. Evans and F. Alegre were supported by the TABULA RASA project funded under the 7th Framework Programme of the European Union (grant agreement number 257289).}% <-this % stops a space
...
the context of automatic speaker verification (ASV). The implementation of replay attacks require no specific expertise nor any sophisticated equipment and thus they arguably present a greater risk than voice conversion or speech synthesis.  The comparative threat of each attack is verified in this paper against seven different ASV systems including a state-of-the-art iVector-PLDA system.  Despite the lack of attention in the literature, experiments show that low-effort replay attacks provoke higher equal error rates (EER) than comparatively higher-effort spoofing attacks such as voice conversion and speech synthesis. Surprisingly, techniques such as score normalisation in some circumstances turned out to work in favour of the replay spoofer. The paper verifies also the effectiveness of two replay countermeasures. Proposed method based on local binary patterns (LBP) yielded better results than detection of far-field recordings.
\end{abstract}

% Note that keywords are not normally used for peerreview papers.
\begin{IEEEkeywords}
speaker verification, spoofing, replay, iVector-PLDA, spoofing countermeasure, local binary patterns.
\end{IEEEkeywords}
...
\subsection{Voice conversion and speech synthesis setup}

The experiments with attacks using voice conversion were conducted with our implementation of the approach originally proposed in~\cite{Matrouf2005}. We again considered the worst-case scenario where the attacker/spoofer has full prior knowledge of the ASV system, and so the front-end processing used in voice conversion was exactly the same as that used for ASV. The filtering model and filter $H_x(f)$ used 19 LPCC and LPC coefficients, respectively.
