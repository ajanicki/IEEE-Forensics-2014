...
One such approach, based upon the detection of so-called pop noise, is proposed in~\cite{Shiota2015}.  Pop noise is a {\bfseries low-frequency component of the speech signal introduced by spontaneous strong breathing during articulation. Pop noise tends to be attenuated upon replay as a result of poor reproduction of low frequencies by speakers} and thus its absence can be used as an indication of replay attacks.  {\bfseries The authors proposed two methods of measuring the pop-noise. In the first one, called single-channel method, low-frequency energy of the input speech signal is measured. In the second one, called double-channel, speech is captured by two microphones, with one of them having a pop-filter, and then the resulting two speech signals are subtracted.}  The results presented in~\cite{Shiota2015} showed that replay attacks can be detected with an EER of less than 4\% using such a detection approach when speech is captured with a {\bfseries high-quality} headset.  Unfortunately, the same approach is not likely to be of use in the case of telephony speech; pop noise is in any case attenuated as a result of narrowband transmission.
